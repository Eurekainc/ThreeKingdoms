...
public class Water : MonoBehaviour {
    ...
    public float deltaAdjust = 1.0f;

    public float waterLeft = -10;
	public float waterWidth = 20;
	public float waterTop = 0;
	public float waterBottom = -3;
        ...
        for (int j = 0; j < 8; j++)
        {
            for (int i = 0; i < xpositions.Length; i++)
            {
                ...
                if (i > 0)
                {
					...
					velocities[i - 1] += leftDeltas[i] * deltaAdjust;
                }
                if (i < xpositions.Length - 1)
                {
                    rightDeltas[i] = spread * (ypositions[i] - ypositions[i + 1]);
					velocities[i + 1] += rightDeltas[i] * deltaAdjust;
                }
            }
            ...
            {
                if (i > 0)
					ypositions[i-1] += leftDeltas[i] * deltaAdjust;
                if (i < xpositions.Length - 1)
					ypositions[i + 1] += rightDeltas[i] * deltaAdjust;
            }
        }
